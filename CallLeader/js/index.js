...
var Game = function () {
    function Game() {
        ...
        this.colors = ["#FFB11B", "#00FFFF", "#FFFFFF","#0000FF","#FFFF00","#FF0000","#800080","#ADFF2F","#FFC0CB"]; // 这里添加颜色
    ...
    Game.prototype.generateBall = function generateBall() {
        ...
        $('.scene .ball-holder').append('<div class="ball honoka" id="ball"></div>');
        this.bounce();
    };
    ...
    * Display score
    ...
    Game.prototype.showGrade = function showGrade(score) {
        if (score > 30) return "真正的拉拉人";else if (score > 25) return "Spiritual!";else if (score > 16) return "哈拉休!";else if (score > 10) return "Fight哒哟!";else if (score > 6) return "Perfect!";else if (score > 2) return "Great!";else if (score > 0) return "Good!";else return "？";
    };
    ...
    Game.prototype.bounce = function bounce() {
        ...
        this.balltween.to('#ball', this.time / 2, { y: '+=250px', scaleY: 0.7, transformOrigin: "bottom", ease: Power2.easeIn,
            ...
            onStart: function onStart() {
				...
				//这里添加颜色
                $('#ball').removeClass('honoka').removeClass('eli').removeClass('kotori').removeClass('umi').removeClass('rin').removeClass('maki').removeClass('nozomi').removeClass('hanayo').removeClass('nico').addClass(new Color().colorcodeToName(_this4.color));
            }
        });
    };
...
var Color = function () {
    function Color() {
        ...
        this.colors = ["#FFB11B", "#00FFFF", "#FFFFFF", "#0000FF","#FFFF00","#FF0000","#800080","#ADFF2F","#FFC0CB"];
    }

    Color.prototype.getRandomColor = function getRandomColor() {
        var colorIndex = Math.random() * 9;
        var color = this.colors[Math.floor(colorIndex)];
        return color;
    };
	//这里添加颜色
    Color.prototype.colorcodeToName = function colorcodeToName(color) {
        var colors = ["#FFB11B", "#00FFFF", "#FFFFFF", "#0000FF","#FFFF00","#FF0000","#800080","#ADFF2F","#FFC0CB"];
        var names = ["honoka", "eli", "kotori", "umi", "rin", "maki", "nozomi", "hanayo", "nico"];
    ...
    Color.prototype.changeColor = function changeColor(el) {
		...
		//这里添加颜色
        el.removeClass('honoka').removeClass('eli').removeClass('kotori').removeClass('umi').addClass(this.colorcodeToName(this.colors[index]));
    ...
    Color.getColorFromClass = function getColorFromClass(el) {
        ...
        for (var i = 0, len = classes.length; i < len; i++) {
			//这里添加颜色
            if (classes[i] == 'honoka' || classes[i] == 'eli' || classes[i] == 'kotori'|| classes[i] == 'umi'|| classes[i] == 'rin'|| classes[i] == 'maki'|| classes[i] == 'nozomi'|| classes[i] == 'hanayo'|| classes[i] == 'nico') {
                return classes[i];
            }
        }
    };
